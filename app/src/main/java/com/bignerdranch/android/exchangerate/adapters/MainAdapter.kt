...
import com.bignerdranch.android.exchangerate.model.CurrencyData

class MainAdapter: RecyclerView.Adapter<MainAdapter.MainViewHolder>() {
    var listMain = emptyList<CurrencyData>()

    class MainViewHolder(view: View): RecyclerView.ViewHolder(view){
        private val binding = ItemBinding.bind(view)
        fun bind(currency: CurrencyData) {
            binding.apply {
                tvCurrency.text = currency.data.USDRUB
                tvValue.text = currency.Valute.USD.Value.toString()
            }
        }
    }
